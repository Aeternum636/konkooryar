...
export default function App() {
  ...
  const saveProfile = useCallback(async (data) => {
    setProfile(data);
    setScreen('dashboard');
    try { await storage.set('profile', JSON.stringify(data)); } catch (e) { setStorageError(true); }
  }, []);

  const changeLevel = useCallback(async (lv) => {
    setLevel(lv);
    try { await storage.set('goalLevel', lv); } catch (e) {}
  }, []);

  const addExam = useCallback(async (exam) => {
    setExams((prev) => {
      const next = [exam, ...prev];
      storage.set('exams', JSON.stringify(next)).catch(() => setStorageError(true));
      return next;
    });
  }, []);
  ...
  const addSheet = useCallback(async (sheet) => {
    setSheets((prev) => {
      ...
      storage.set('testsheets', JSON.stringify(next)).catch(() => setStorageError(true));
      return next;
    });
  }, []);
  ...
  const addBudget = useCallback(async (budget) => {
    setBudgets((prev) => {
      ...
      storage.set('budgets', JSON.stringify(next)).catch(() => setStorageError(true));
      return next;
    });
  }, []);
  ...
  const addResource = useCallback(async (resource) => {
    setResources((prev) => {
      ...
      storage.set('resources', JSON.stringify(next)).catch(() => setStorageError(true));
      return next;
    });
  }, []);
  ...
  const updateTaskDay = useCallback(async (dateKey, tasks) => {
    setTasksByDate((prev) => {
      ...
      storage.set('tasksByDate', JSON.stringify(next)).catch(() => setStorageError(true));
      return next;
    });
  }, []);
  ...
  const saveMood = useCallback(async (dateKey, entry) => {
    setMoodByDate((prev) => {
      ...
      storage.set('moodByDate', JSON.stringify(next)).catch(() => setStorageError(true));
      return next;
    });
  }, []);
